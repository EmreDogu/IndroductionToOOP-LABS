public class TestCafe {
	public static void main(String args[]){
		...
		CaffeineBeverage cBeverage = new Latte(Beverage.SMALL, true);
			order.add(new OrderItem(cBeverage, 2));
			
		cBeverage = new Cappucino(Beverage.MEDIUM, false);
			order.add(new OrderItem(cBeverage, 2));
			
		cBeverage = new Americano(Beverage.LARGE, true);
			order.add(new OrderItem(cBeverage, 1));
			
			

		TeaBeverage tBeverage = new BlackTea(Beverage.SMALL, false);
			order.add(new OrderItem(tBeverage, 3));

		tBeverage = new GreenTea(Beverage.MEDIUM,true);
			order.add(new OrderItem(tBeverage, 1));

		tBeverage = new LindenTea(Beverage.SMALL,true);
			order.add(new OrderItem(tBeverage, 1));
